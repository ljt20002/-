...
- **提示词优化**: 内置一键优化功能，可调用专用模型将简短的草稿增强为结构化、专业级的提示词。
- **模型对比**: 支持同时选择多个模型进行对话，一键对比不同模型的回复效果，并支持批量复制对比结果。
- **消耗统计**: 实时显示每次对话的 Token 消耗及预估成本（基于不同模型的计费标准）。
- **现代化 UI**: 采用 Ant Design 5.x 组件库结合 Tailwind CSS 设计，支持响应式布局，适配移动端。
...
- **纯前端架构**: 无后端服务，所有数据存储在本地浏览器（IndexedDB/LocalStorage），确保隐私安全。
- **UI 框架**: 使用 **Ant Design 5.x** 提供高质量的交互组件，结合 **Tailwind CSS** 进行灵活的样式定制。
